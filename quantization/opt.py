...
def opt_sequential(model, dataloader, dev, args):
    ...
    print("layer[0] is",layers[0])

    dtype = next(iter(model.parameters())).dtype
    print("model data type",dtype)
    ...
    for i in tqdm(range(len(layers))):
        ...
        # Convert self-attention components to float32
        layer.self_attn.q_proj.weight.data = layer.self_attn.q_proj.weight.data.float()
        layer.self_attn.k_proj.weight.data = layer.self_attn.k_proj.weight.data.float()
        layer.self_attn.v_proj.weight.data = layer.self_attn.v_proj.weight.data.float()
        layer.self_attn.out_proj.weight.data = layer.self_attn.out_proj.weight.data.float()

        # Convert fc1 and fc2 weights to float32
        layer.fc1.weight.data = layer.fc1.weight.data.float()
        layer.fc2.weight.data = layer.fc2.weight.data.float()
        ...
        for j in range(args.nsamples):
            ...
            outs[j] = layer(inps[j].unsqueeze(0).to(torch.float32),
                            attention_mask=attention_mask)[0]
